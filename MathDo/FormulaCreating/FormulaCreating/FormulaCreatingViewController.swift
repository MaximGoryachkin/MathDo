...
protocol VariableDisplayProtocol: AnyObject {
...
final class FormulaCreatingViewController: UIViewController {
    ...
    private func showSavingVC() {
        ...
        FormulaReader.shared.verifyFormulaSyntax(expression: formulaCreatingView.formulaTextField.text ?? "") { success, sender, error in
            ...
            var formulaSavingVC: FormulaSavingViewController!

            switch savingType {
            case .creating:
                formula = FormulaModel(context: DatabaseManager.shared.getContext())
                formulaSavingVC = FormulaSavingViewController(formulaCreatingVC: self, savingType: .creating)
                
            case .editing(let formula):
                formulaSavingVC = FormulaSavingViewController(formulaCreatingVC: self, savingType: .editing(formula: formula))
            }
            ...
            formulaSavingVC.delegate = self
            
            if let presentationController = formulaSavingVC.presentationController as? UISheetPresentationController  {
                presentationController.detents = [.medium()]
            }
            formulaSavingVC.modalPresentationStyle = .custom
        
            present(formulaSavingVC, animated: true)
